...
class EnhancedContractAnalyzer:
    ...
    def _scan_full_text_patterns(self, text: str) -> List[EnhancedRiskItem]:
        ...
        for pattern_info in cross_boundary_patterns:
            ...
            for match in matches:
                # Extract surrounding context by finding nearest periods
                start = text.rfind('.', 0, match.start())
                start = start + 1 if start != -1 else max(0, match.start() - 100)
                
                end = text.find('.', match.end())
                end = end + 1 if end != -1 else min(len(text), match.end() + 100)
    ...
    async def analyze_contract(self, text: str, user_role: str = "Neutral Observer") -> Dict:
        ...
        try:
            ...
            # Step 3: Enhanced AI analysis (PARALLELIZED)
            ...
            for risk in unique_risks:
                if gemini_available and risk in top_risks:
                    # Queue async task
                    tasks.append(self._enhance_with_comprehensive_ai(risk, text, user_role))
                    results_order.append(True)
                else:
                    results_order.append(self._convert_to_api_format(risk))
            
            enhanced_risks = []
            if tasks:
                # Run all 5 Gemini calls at the exact same time
                ai_results = await asyncio.gather(*tasks)
                ai_index = 0
                for is_ai_task in results_order:
                    if is_ai_task is True:
                        enhanced_risks.append(ai_results[ai_index])
                        ai_index += 1
                    else:
                        enhanced_risks.append(is_ai_task)
            else:
                enhanced_risks = [r for r in results_order]
    ...
    async def _enhance_with_comprehensive_ai(self, risk: EnhancedRiskItem, full_text: str, user_role: str) -> Dict:
        ...
        try:
            ...
            # Run the synchronous SDK call in a separate thread so it doesn't freeze FastAPI
